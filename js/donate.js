...
async function getFundCnt() {
    ...
    let promise = new Promise((res, rej) => {
        ...
        Chiron.fundCnt(function(error, result) {
            if (!error)
                res(result);
            else{
                rej(false);
...
async function getFundDetails() {
    ...
    let promise = new Promise(async (res, rej) => {
        ...
        for (var i=0;i<fundCnt;i++){
            Chiron.Funds(i,function(error, result) {
                if (!error){
...
async function donate(_orgID) {
    ...
    let promise = new Promise((res, rej) => {
        ...
        const _amount = parseFloat(document.getElementById(`donationAmount${_orgID}`).value);
        Chiron.donateToFund(parseInt(_orgID), {value: web3.toWei(_amount, 'ether')},function(error, result) {
            if (!error)
                res(result);
            else{
                rej(false);
